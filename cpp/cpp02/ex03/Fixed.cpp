...
{
	Fixed result;
	result.setRawBits(raw_fixed + rhs.raw_fixed);
	return result;
}
...
Fixed Fixed::operator*(const Fixed& rhs) const
{
	const long long multiplied = static_cast<long long>(raw_fixed) * rhs.raw_fixed;
	Fixed result;
	result.setRawBits((multiplied >> fraction) + (multiplied >> (fraction - 1) & 1));
	return result;
}
...
{
	Fixed result;
	result.setRawBits((static_cast<long long>(raw_fixed) << fraction) / rhs.raw_fixed);
	return result;
}
