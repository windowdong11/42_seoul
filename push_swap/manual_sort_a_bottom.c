#include "utils/stack_ab.h"

static void	sort_a_bottom_order(t_stack_ab *st, int order)
{
	if (order == 123)
	{
		st->pb(st, 1);
		st->sa(st, 1);
		st->ra(st, 1);
		st->ra(st, 1);
		st->pa(st, 1);
		st->ra(st, 1);
	}
	else if (order == 132)
	{
		st->sa(st, 1);
		st->ra(st, 1);
		st->sa(st, 1);
		st->ra(st, 1);
		st->ra(st, 1);
	}
	else if (order == 231)
	{
		st->sa(st, 1);
		st->ra(st, 1);
		st->ra(st, 1);
		st->ra(st, 1);
	}
}

static void	sort_a_bottom_order2(t_stack_ab *st, int order)
{
	if (order == 213)
	{
		st->pb(st, 1);
		st->sa(st, 1);
		st->ra(st, 1);
		st->pa(st, 1);
		st->ra(st, 1);
		st->ra(st, 1);
	}
	else if (order == 312)
	{
		st->ra(st, 1);
		st->sa(st, 1);
		st->ra(st, 1);
		st->ra(st, 1);
	}
	else if (order == 321)
	{
		st->rb(st, 1);
		st->rb(st, 1);
		st->rb(st, 1);
	}
}

void	manual_sort_a_bottom(t_stack_ab *st, int (*cmp)(int, int))
{
	if (cmp(st->stack_a->top->value, st->stack_a->top->next->value))
	{
		if (cmp(st->stack_a->top->next->value,
				st->stack_a->top->next->next->value))
			sort_a_bottom_order(st, 123);
		else if (cmp(st->stack_a->top->value,
				st->stack_a->top->next->next->value))
			sort_a_bottom_order(st, 132);
		else
			sort_a_bottom_order(st, 231);
	}
	else
	{
		if (cmp(st->stack_a->top->value, st->stack_a->top->next->next->value))
			sort_a_bottom_order2(st, 213);
		else if (cmp(st->stack_a->top->next->value,
				st->stack_a->top->next->next->value))
			sort_a_bottom_order2(st, 312);
		else
			sort_a_bottom_order2(st, 321);
	}
}
